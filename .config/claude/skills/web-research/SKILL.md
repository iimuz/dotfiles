...
5. Trace check。読み取り専用の検証サブエージェントを 1 つ立て、`sources/` 配下の全ノートと
   `report.md` を実際に読んで突き合わせさせる。確認するのは、`report.md` のすべての主張に
   `[NN]` 引用があること、すべての `[NN]` が実在するノートを指すこと、ノートに無い情報が
...
6. Adversarial verification。ユーザーが明示的に要求した場合のみ実行する。主要な主張ごとに
   ...
   その出典にも要約サブエージェントを立ててノートを書かせる。`NN` は既存の連番の続きを
   オーケストレーターが割り当てる。`report.md` の該当する主張に `[NN]` 付きで注記する。
   ノートを作らないまま `report.md` に追記しない。
