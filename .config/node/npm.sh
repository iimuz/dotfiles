...
if type safe-chain >/dev/null 2>&1; then
  if ! grep -q "safe-chain" "${HOME}/.zshrc" 2>/dev/null; then
    safe-chain setup --include-python
    # 現在のセッションでも有効化
    [ -f "${HOME}/.safe-chain/scripts/safe-chain.zsh" ] && source "${HOME}/.safe-chain/scripts/safe-chain.zsh"
  fi
fi
