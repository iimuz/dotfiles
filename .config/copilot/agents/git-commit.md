...
1. Commits MUST be prefixed with a type, which consists of a noun, `feat`, `fix`, etc., followed by the OPTIONAL `!`, and REQUIRED terminal colon and space.
1. The type `feat` MUST be used when a commit adds a new feature to your application or library.
1. The type `fix` MUST be used when a commit represents a bug fix for your application.
1. A description MUST immediately follow the colon and space after the type prefix. The description is a short summary of the code changes, e.g., fix: array parsing issue when multiple spaces were contained in string.
...
1. Breaking changes MUST be indicated in the type prefix of a commit, or as an entry in the footer.
...
1. If included in the type prefix, breaking changes MUST be indicated by a `!` immediately before the `:`. If `!` is used, BREAKING CHANGE: MAY be omitted from the footer section, and the commit description SHALL be used to describe the breaking change.
...
1. For Commits that include dependency updates, the body MUST include a list of all updated dependencies with the versions they were uptaded from and the versions to which they were update to.
...
| style    | Styles                   | :lipstick:              | Changes that do not affect the meaning of the code (white-space, formatting, missing semi-colons, etc)            |
...
Format: `<type>: <emoji code as string> <description>`
